 ...
 * it on your own
 * development machine. To prevent other people from using this module, it is
 * required that
 ...
 * you may need to add
 * the following URLs in your application configuration in order to access
 * GiiModule:
 * <pre>
 * 'components'=>array(
 * 'urlManager'=>array(
 ...
 *
 * @property string $assetsUrl The base URL that contains all published asset
 *           files of gii.
 *          
 * @author Qiang Xue <qiang.xue@gmail.com>
...
class GiiModule extends CWebModule {
	 ...
	 *      If this property is set false, then GiiModule can be accessed
	 *      without password
	...
	/**
	 *
	 * @var array the IP filters that specify which IP addresses are allowed to
	 *      access GiiModule.
	 *      Each array element represents a single filter. A filter can be
	 *      either an IP address
	 *      or an address with wildcard (e.g. 192.168.0.*) to represent a
	 *      network segment.
	 *      If you want to allow all IPs to access gii, you may set this
	 *      property to be false
	 *      (DO NOT DO THIS UNLESS YOU KNOW THE CONSEQUENCE!!!)
	 *      The default value is array('127.0.0.1', '::1'), which means
	 *      GiiModule can only be accessed
	 *      on the localhost.
	 */
	...
	/**
	 *
	 * @var array a list of path aliases that refer to the directories
	 *      containing code generators.
	 *      The directory referred by a single path alias may contain multiple
	 *      code generators, each stored
	 ...
	 *      This value will be used by PHP chmod function.
	 *      Defaults to 0777, meaning the directory can be read, written and
	 ...
	 * This method will check to see if user IP and password are correct if they
	 * attempt
					...
					if (isset ( $generators [$name] ) && is_dir ( "$path/$name/templates" )) {
						...
						foreach ( $dirs as $dir ) {
							if ($dir [0] !== '.' && is_dir ( $templatePath . '/' . $dir ))
								$generators [$name] ['templates'] [$dir] = strtr ( $templatePath . '/' . $dir, array (
										'/' => DIRECTORY_SEPARATOR,
										'\\' => DIRECTORY_SEPARATOR 
								) );
						}
