...
class ModelCode extends CCodeModel {
	...
	/**
	 *
	 * @var array list of candidate relation code. The array are indexed by AR
	 *      class names and relation names.
	 *      Each element represents the code of the one relation in one AR
	 *      class.
	 */
	protected $relations;
	public function rules() {
		return array_merge ( parent::rules (), array (
				array (
						'tablePrefix, baseClass, tableName, modelClass, modelPath',
						'filter',
						'filter' => 'trim' 
				),
				array (
						'tableName, modelPath, baseClass',
						'required' 
				),
				array (
						'tablePrefix, tableName, modelPath',
						'match',
						'pattern' => '/^(\w+[\w\.]*|\*?|\w+\.\*)$/',
				...
				array (
						'tablePrefix, modelClass, baseClass',
						'match',
						'pattern' => '/^[a-zA-Z_]\w*$/',
				...
				array (
						'tablePrefix, modelPath, baseClass, buildRelations',
						'sticky' 
				) 
		) );
	}
	...
	public function prepare() {
		...
		if ($tableName [strlen ( $tableName ) - 1] === '*') {
			...
			$tables = array (
					$this->getTableSchema ( $this->tableName ) 
			);
	 ...
	 * For example mysql allows field name like "2011aa", but PHP does not allow
	 * variable like "$model->2011aa" @param CDbTableSchema $table the table
	 * schema object @return string the invalid table column name. Null if no
	 * error.
	 */
	...
	public function generateLabels($table) {
		...
		foreach ( $table->columns as $column ) {
			$label = ucwords ( trim ( strtolower ( str_replace ( array (
					'-',
					'_' 
			), ' ', preg_replace ( '/(?<![A-Z])[A-Z]/', ' \0', $column->name ) ) ) ) );
		...
		foreach ( Yii::app ()->db->schema->getTables () as $table ) {
			...
			if ($this->isRelationTable ( $table )) {
				...
				foreach ( $table->foreignKeys as $fkName => $fkEntry ) {
					...
					$refTable = $fkEntry [0]; // Table name that current fk references
					                       // to
	 ...
	 * Their PK has 2 fields, and both of those fields are also FK to other
	 * separate tables.
	 * 
	 * @param
	 *        	CDbTableSchema table to inspect
	...
	protected function isRelationTable($table) {
		...
		$table->foreignKeys [$pk [0]] [0] !== $table->foreignKeys [$pk [1]] [0]); // and
		                                                                       // the
		                                                                       // foreign
		                                                                       // keys
		                                                                       // point
	...
	/**
	 * Generate a name for use as a relation name (inside relations() function
	 * in a model).
	 * 
	 * @param
	 *        	string the name of the table to hold the relation
	 * @param
	 *        	string the foreign key name
	 * @param
	 *        	boolean whether the relation would contain multiple objects
